...
export default function CryptoCalculation() {
    ...
    const handleAmount = (item: any) => {
        setValue(item)
        updateCrypto({...data, "coin_amount_to_swap": item+""})
    }

    const { isLoading } = useQuery(["usd-rate"], () => axios.get(`/swap/usd-rate`,
        {
            headers: {
        ...
        onSuccess: (data: any) => { 
            setRate(data?.data?.usdt_ngn_rate)
        }
    }) 
    ...
    return (
        <div className=' w-full lg:mt-0 mt-6 lg:w-[400px] h-fit border-[1.5px] border-[#D0D5DD] rounded-lg bg-white py-9 px-6 ' >
            ...
            <p className=' font-bold text-[#303179] mb-4 ' >{rate && "1 USD = " + (isLoading ? "..." : cashFormat(rate)) + " NGN"}</p>
            <div className=' relative w-full border border-[#94A3B8] h-[60px] mt-4 rounded-lg px-4 lg:px-[26px] flex items-center justify-between ' >
                <div className=' w-full ' >
                    ...
                    <input onFocus={(e) => e.target.addEventListener("wheel", function (e) { e.preventDefault() }, { passive: false })} onChange={(e) => handleAmount(e.target.value)} type='number' className=' w-full border-0 font-medium text-lg outline-none ' placeholder="0" />
                </div>
